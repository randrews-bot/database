...
url = "https://geo.fcc.gov/api/census/block/find" params = {"latitude": lat, "longitude": lng, "format": "json"} async with httpx.AsyncClient(timeout=10) as client: r = await client.get(url, params=params) if r.status_code >= 400: raise HTTPException(status_code=502, detail="FCC FIPS lookup failed") data = r.json() blk = data.get("Block", {}) fips = blk.get("FIPS") if not fips or len(fips) < 11: raise HTTPException(status_code=404, detail="FIPS not found") state = fips[0:2] county = fips[2:5] tract = fips[5:11] # 6-digit tract code return {"state": state, "county": county, "tract": tract}
